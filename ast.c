...
Token ast_peek(ASTParser *parser)
{
...
bool is_end(ASTParser *parser)
{
    return parser->current >= parser->size;
}
...
ASTNode *parse_set_or_dict(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    while (peeked.type != TOK_RCURLY)
    {
        ...
        if (key == NULL)
        {
            report_parse_error(error_code, "Expected item or closing of sequence delcaration");
        ...
        if (is_set == -1)
        {
            ...
            {
                report_parse_error(error_code, "Expected \":\" or \",\" or \"}\" after key in set or dict declaration");
                linked_list_free(keys);
                linked_list_free(values);
                return NULL;
            }
        ...
        if (is_set == 1)
        {
            ...
            {
                report_parse_error(error_code, "Expected \",\" or \"}\" after key in set declaration");
                linked_list_free(keys);
                linked_list_free(values);
                return NULL;
            }
        ...
        if (is_set == 0)
        {
            ...
            {
                report_parse_error(error_code, "Expected \":\" after key in dict declaration");
                linked_list_free(keys);
                linked_list_free(values);
                return NULL;
            }
            ...
            if (value == NULL)
            {
                report_parse_error(error_code, "Expected value after \":\" in set declaration");
                linked_list_free(keys);
                linked_list_free(values);
                return NULL;
            }
            ...
            if (peeked.type != TOK_RCURLY && peeked.type != TOK_COMMA)
            {
                report_parse_error(error_code, "Expected \"}\" or \",\" after value in dict declaration");
                linked_list_free(keys);
                linked_list_free(values);
                return NULL;
            }
...
ASTNode *parse_list_or_tuple(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    ASTNode *node = create_ast_node(parser, (peeked.type == TOK_LBRACKET) ? AST_LIST : AST_TUPLE);
    ...
    while (peeked.type != closing)
    {
        ...
        {
            report_parse_error(error_code, "Expected \",\" or \")\" after sequence item");
            linked_list_free(node->data.sequence.items);
            my_free(parser->pool, node);
            return NULL;
        }
...
ASTNode *parse_lambda(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    {
        report_parse_error(error_code, "Expected lambda arguments or \":\" after \"lambda\"");
        return NULL;
    }
    ...
    if (expr == NULL)
    {
        report_parse_error(error_code, "Expected expression or \"pass\" after lambda arguments");
        return NULL;
    }
...
ASTNode *parse_function_call_partial(ASTParser *parser, ParseErrorCode *error_code)
{
    if (is_end(parser))
    {
        return NULL;
    ...
    {
        report_parse_error(error_code, "Expected expression list or \":\" after \"(\"");
        return NULL;
    }
...
ASTNode *parse_index_or_slice_partial(ASTParser *parser, ParseErrorCode *error_code)
{
    if (is_end(parser))
    {
        return NULL;
    ...
    {
        my_free(parser->pool, start);
        report_parse_error(error_code, "Expected \"]\" or \":\" after index start");
        return NULL;
    }
    ast_consume(parser);
    ...
    if (end == NULL)
    {
        ...
        my_free(parser->pool, start);
        report_parse_error(error_code, "Expected expression or \"]\" after \":\"");
        return NULL;
    }
    if (is_end(parser))
    {
...
ASTNode *parse_product(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    Token ast_peeked = ast_peek(parser);
    while (ast_peeked.type == TOK_MUL || ast_peeked.type == TOK_DIV || ast_peeked.type == TOK_FLOORDIV || ast_peeked.type == TOK_MOD)
    {
        ast_consume(parser);
...
ASTNode *parse_comparison(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    Token ast_peeked = ast_peek(parser);
    if (
        ast_peeked.type == TOK_LT || ast_peeked.type == TOK_LEQ || ast_peeked.type == TOK_GT || ast_peeked.type == TOK_GEQ || ast_peeked.type == TOK_EQ)
    {
        ast_consume(parser);
...
ASTNode *parse_logical(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    Token ast_peeked = ast_peek(parser);
    while (
        ast_peeked.type == TOK_AND || ast_peeked.type == TOK_OR)
    {
        ast_consume(parser);
...
ASTNode *parse_ternary(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    if (is_end(parser))
    {
        report_parse_error(error_code, "Unexpected end of input, expected \"else\" after ternary if");
        return NULL;
    }
    peeked = ast_peek(parser);
    ...
    if (!else_body)
    {
        report_parse_error(error_code, "Expected expression after \"else\" in ternary if");
        return NULL;
    }
...
ASTNode *parse_expression_stripped(ASTParser *parser, ParseErrorCode *error_code)
{
...
ASTNode *parse_expression(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    while (true)
    {
        ASTNode *index_or_slice = parse_index_or_slice_partial(parser, error_code);
        if (index_or_slice != NULL)
        {
            ...
            ASTNode *new_node = create_ast_node(parser, (index_or_slice->type == AST_SLICE_PARTIAL) ? AST_SLICE : AST_INDEX);
            new_node->data.index_or_slice.index = index_or_slice;
            new_node->data.index_or_slice.subject = node;
            node = new_node;
            continue;
        }
        ASTNode *function_call = parse_function_call_partial(parser, error_code);
        if (function_call)
        {
            if (*error_code != PARSE_SUCCESS)
            {
...
{
    while (!is_end(parser) && ast_peek(parser).type == TOK_INDENT || ast_peek(parser).type == TOK_NEWLINE)
    {
...
void retract_indentation(ASTParser *parser)
{
    parser->current_indentation--;
}

ASTNode *parse_condition_partial(ASTParser *parser, TokenType conditional_type, bool needs_condition, ParseErrorCode *error_code)
{
    if (is_end(parser))
    {
        report_parse_error(error_code, "Unexpected end of input");
    ...
    ast_skip_indentation(parser);
    while (!is_end(parser) && parser->current_indentation == current_indentation + 1)
    {
        ASTNode *statement = parse_statement(parser, error_code);
        if (!statement)
        {
...
ASTNode *parse_if_statement(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    int current_indentation = parser->current_indentation;
    ASTNode *if_part = parse_condition_partial(parser, TOK_IF, true, error_code);
    if (!if_part)
    {
        return NULL;
    }
    ...
    while (!is_end(parser))
    {
        ...
        ASTNode *elif_part = parse_condition_partial(parser, TOK_ELIF, true, error_code);
        if (*error_code != PARSE_SUCCESS)
        {
    ...
    ASTNode *else_part = parse_condition_partial(parser, TOK_ELSE, false, error_code);
    if (*error_code != PARSE_SUCCESS)
    {
        linked_list_free(node->data.if_statement.elifs);
        my_free(parser->pool, node);
...
ASTNode *parse_for_loop(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    {
        report_parse_error(error_code, "Expected statement or \"pass\" after for");
        my_free(parser->pool, identifier);
        my_free(parser->pool, iterable);
        return NULL;
    }
    ...
    ast_skip_indentation(parser);
    while (!is_end(parser) && current_indentation + 1 == parser->current_indentation)
    {
        ASTNode *statement = parse_statement(parser, error_code);
        if (statement == NULL)
        {
...
ASTNode *parse_while(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    {
        report_parse_error(error_code, "Expected condition or \":\" after \"while\"");
        return NULL;
    }
    if (ast_peek(parser).type != TOK_CLR)
    {
    ...
    if (first_statement == NULL)
    {
        report_parse_error(error_code, "Expected statement or \"pass\" after while");
        my_free(parser->pool, condition);
        return NULL;
    }
    ...
    {
        ast_skip_indentation(parser);
        if (is_end(parser) || parser->current_indentation != current_indentation + 1)
        {
            break;
        }
        ASTNode *statement = parse_statement(parser, error_code);
    ...
    if (parser->current_indentation > current_indentation + 1)
    {
        ...
        my_free(parser->pool, condition);
        linked_list_free(node->data.while_loop.body);
        report_parse_error(error_code, "Unexpected indentation after while loop");
        return NULL;
    }
    while (parser->current_indentation)
    {
...
ASTNode *parse_function_definition(ASTParser *parser, ParseErrorCode *error_code)
{
    if (is_end(parser))
    {
        return NULL;
    ...
    if (ast_peek(parser).type != TOK_CLR)
    {
        report_parse_error(error_code, "Expected \":\" after function definition");
        my_free(parser->pool, identifier);
        my_free(parser->pool, argument_list);
        return NULL;
    }
    ...
    if (parser->current_indentation > current_indentation + 1)
    {
        ...
        my_free(parser->pool, argument_list);
        linked_list_free(node->data.function_def.body);
        report_parse_error(error_code, "Unexpected indentation after function definition loop");
        return NULL;
    }
    while (parser->current_indentation)
    {
...
ASTNode *parse_assign(ASTParser *parser, ParseErrorCode *error_code)
{
    ...
    while (true)
    {
        ...
        if (peeked_type != TOK_ASSIGN && peeked_type != TOK_AUGPLUS && peeked_type != TOK_AUGMINUS && peeked_type != TOK_AUGMUL && peeked_type != TOK_AUGDIV && peeked_type != TOK_AUGMOD && peeked_type != TOK_AUGPOW)
        {
            return target;
        }
        ast_consume(parser);
...
ASTNode *parse_unary_statement(ASTParser *parser, TokenType ttype, ASTNodeType atype, ParseErrorCode *error_code)
{
    if (is_end(parser))
    {
        return NULL;
    ...
    {
        my_free(parser->pool, node);
        report_parse_error(error_code, "Expected expression after unary operator");
        return NULL;
    }
    return node;
}
